...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		//loop do, if frontier is empty, return failure
		while(!frontier.isEmpty()){
			...
			//get child states to create new search nodes
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(node.getState());
			//for each state from the child nodes reachable by current node
			for(GridPos gp : childStates) {
				SearchNode childNode = new SearchNode(gp, node);
